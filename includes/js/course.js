...
function ajaxsubmit_comments(){
  ...
  commentform.submit(function(){
    ...
    statusdiv.html('<p>'+qm_course_module_strings.processing+'</p>');
    ...
    $.ajax({
      ...
      error: function(XMLHttpRequest, textStatus, errorThrown){
        ...
        statusdiv.html('<p class="wdpajax-error">'+qm_course_module_strings.too_fast_answer+'</p>');
      },
      success: function(data, textStatus){
        $('#question').css('opacity',1);
        $('#ajaxloader').addClass('disabled');
        if(data=="success"){
          statusdiv.html('<p class="ajax-success" >'+qm_course_module_strings.answer_saved+'</p>');
          $('#ques'+qid).addClass('done');
        }
        else
          statusdiv.html('<p class="ajax-error" >'+qm_course_module_strings.saving_answer+'</p>');
...
jQuery(document).ready( function($) {
	...
	$("#average .dial").knob({
	    ...
	    'width': 120, 
	    'height': 120, 
	    'fgColor': qm_course_module_strings.theme_color, 
	    'bgColor': '#f6f6f6',   
	    'thickness': 0.1
	});
	$("#pass .dial").knob({
...
$( 'body' ).delegate( '.remove_user_course','click',function(event){
      ...
      var $this = $(this);
      $.confirm({
          text: qm_course_module_strings.remove_user_text,
          confirm: function() {
             $.ajax({
                    type: "POST",
                    url: ajaxurl,
          ...
          confirmButton: qm_course_module_strings.remove_user_button,
          cancelButton: qm_course_module_strings.cancel
      });
	});
...
$( 'body' ).delegate( '.reset_course_user','click',function(event){
      ...
      var $this = $(this);
      $.confirm({
        text: qm_course_module_strings.reset_user_text,
          confirm: function() {
          $.ajax({
                  type: "POST",
                  url: ajaxurl,
         ...
         cancel: function() {
          ...
          confirmButton: qm_course_module_strings.reset_user_button,
          cancelButton: qm_course_module_strings.cancel
        });
	});
...
$( 'body' ).delegate( '.course_stats_user', 'click', function(event){
  ...
  $('.reset_quiz_user').click(function(event){
      ...
      var $this = $(this);
      $.confirm({
          text: qm_course_module_strings.quiz_rest,
          confirm: function() {
      ...
      $.ajax({
       ...
       cancel: function() {
        ...
        confirmButton: qm_course_module_strings.quiz_rest_button,
        cancelButton: qm_course_module_strings.cancel
      });
  });
...
$( 'body' ).delegate( '.give_marks', 'click', function(event){
	    ...
	    $.ajax({
	            ...
	            success: function (html) {
	                $this.find('i').remove();
	                $this.html(qm_course_module_strings.marks_saved);
	            }
	    });
});
...
$( 'body' ).delegate( '#mark_complete', 'click', function(event){
    ...
    $.ajax({
            ...
            success: function (html) {
                $this.find('i').remove();
                $this.html(qm_course_module_strings.quiz_marks_saved);
            }
    });
});
	...
	$('.begin_quiz').click(function(event){
	    ...
	    $.ajax({
                ...
                $('.begin_quiz').each(function(){
                    ...
                    $(this).text(qm_course_module_strings.submit_quiz);
                });
            }
        });
	});
...
$( 'body' ).delegate( '#send_course_message', 'click', function(event){
  ...
  $this.html('<i class="icon-sun-stroke animated spin"></i> '+qm_course_module_strings.sending_messages);
  var i=0;
...
$( 'body' ).delegate( '#add_student_to_course', 'click', function(event){
  ...
  if(students.length <= 0){ 
    $('#add_student_to_course').html(qm_course_module_strings.unable_add_students);
    setTimeout(function(){$this.html(defaultxt);}, 2000);
    return;
  }

  $this.html('<i class="icon-sun-stroke animated spin"></i>'+qm_course_module_strings.adding_students);
  var i=0;
  $.ajax({
        type: "POST",
        url: ajaxurl,
        ...
        success: function (html) {
          ...
          if(html.length && html !== '0'){
            $('#add_student_to_course').html(qm_course_module_strings.successfuly_added_students);
...
$( 'body' ).delegate( '#assign_course_badge_certificate', 'click', function(event){
  ...
  var $this = $(this);
  var defaultxt=$this.html();
  $this.html('<i class="icon-sun-stroke animated spin"></i> '+qm_course_module_strings.processing);
  var i=0;
  $('.member').each(function(){
    if($(this).is(':checked')){
      members[i]=$(this).val();
