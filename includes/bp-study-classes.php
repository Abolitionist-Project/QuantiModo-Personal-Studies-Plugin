 ...
 * BP_study_Highfive object in get()) is one suggested implementation.
 */
...
class BP_STUDY {
	...
	function save() {
		...
		do_action( 'bp_study_data_before_save', $this );
	...
	/**
	 * Part of our bp_study_has_high_fives() loop
	 *
	 * @package BuddyPress_Study_Component
	 * @since 1.6
	 */
	 ...
	 *
	 * Look at bp-core-classes.php for studies of mass delete.
	 */
	...
	public static function study_exists( $slug) {
		global $post;
		...
		if(get_post_type( $my_posts[0]->ID ) == BP_STUDY_SLUG){
			return true;
		}else
			return false;

		//global $wpdp;
		//return $wpdb->get_var( $wpdb->prepare( "SELECT id FROM wp_posts WHERE slug = %s && post_status = 'publish' && post_type = 'study'", strtolower( $slug ) ) );
	}
