<?php 
/**
 * QuantiPress Protected API
 *
 * Wrapper class for the QuantiPress marketplace protected API methods specific
 * to the QuantiPress WordPress Toolkit plugin.
 *
 * @package     WordPress
 * @subpackage  QuantiPress WordPress Toolkit
 * @author      Derek Herman <derek@QuantiPress.com>
 * @since       1.0
 */ 
class Envato_Protected_API {
   ...
   * @since     1.0
   */
  protected $public_url = 'http://marketplace.QuantiPress.com/api/edge/set.json';
  ...
  public function __construct( $user_name = '', $api_key = '' ) {
    ...
    if ( $user_name == '' ) {
      $this->set_error( 'user_name', __( 'Please enter your QuantiPress Marketplace Username.', 'qm' ) );
    }
      
    if ( $api_key == '' ) {
      $this->set_error( 'api_key', __( 'Please enter your QuantiPress Marketplace API Key.', 'qm' ) );
    }
  ...
  public function private_user_data( $set = '', $user_name = '', $set_data = '', $allow_cache = false, $timeout = 300 ) { 
    ...
    if ( $set == '' ) {
      $this->set_error( 'set', __( 'The API "set" is a required parameter.', 'qm' ) );
    }
    ...
    $url = "http://marketplace.QuantiPress.com/api/edge/$user_name/$this->api_key/$set$set_data.json";
  ...
  public function wp_download( $item_id ) {
    ...
    if ( ! isset( $item_id ) ) {
      $this->set_error( 'item_id', __( 'The QuantiPress Marketplace "item ID" is a required parameter.', 'qm' ) );
    }
...
/* End of file class-QuantiPress-api.php */
/* Location: ./includes/class-QuantiPress-api.php */